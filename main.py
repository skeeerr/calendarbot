...
API_TOKEN = '7681033601:AAFNOM_VzCzwJ8jkduaGgld01HxykO3ii1w'
ADMIN_ID = 852861796
...
async def buy_subscription(callback_query: types.CallbackQuery):
    ...
    subscriptions = [
        ("3 дня - 40₽", "3 дня", "40₽"),
        ("Неделя - 100₽", "1 неделя", "100₽"),
        ("Месяц - 400₽", "1 месяц", "400₽"),
        ("2 месяца - 600₽", "2 месяца", "600₽"),
        ("3 месяца - 650₽", "3 месяца", "650₽"),
        ("6 месяцев - 1200₽", "6 месяцев", "1200₽"),
        ("10 месяцев - 1500₽", "10 месяцев", "1500₽"),
        ("12 месяцев - 1600₽", "12 месяцев", "1600₽"),
    ]
    for label, period, price in subscriptions:
        kb.add(InlineKeyboardButton(label, callback_data=f"sub_{period}_{price}"))
    await bot.send_message(callback_query.from_user.id, "Выберите срок подписки:", reply_markup=kb)

@dp.callback_query_handler(lambda c: c.data.startswith('sub_'))
async def subscription_info(callback_query: types.CallbackQuery):
    _, period, price = callback_query.data.split('_', 2)
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("🤖 CryptoBot", url="https://t.me/CryptoBot"))
    kb.add(InlineKeyboardButton("⭐ Telegram Stars", url="https://t.me/StellarStarsBot"))
    kb.add(InlineKeyboardButton("💳 Рублями", url="https://t.me/CertificateSelling_bot"))
    await bot.send_message(
        callback_query.from_user.id,
        f"Подписка — Календарь-напоминалка\nСрок: {period}\nСумма: {price}\n\nОплата:",
        reply_markup=kb
    )
